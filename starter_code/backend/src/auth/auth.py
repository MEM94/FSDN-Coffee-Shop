...
AUTH0_DOMAIN = 'dev-fsnd-coffee.us.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'image'
...
## Auth Header from Auth0 Documentation
...
def check_permissions(permission, payload):
    if 'permissions' not in payload:
        raise AuthError({"code": "invalid","description": "Permissions are not included in the payload"}, 400)

    if permission not in payload['permissions']:
        raise AuthError({"code": "invalid","description": "Permission not found"}, 402)
    return True

## verify_decode_jwt from Auth0 Documentation
